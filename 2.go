...
func (i *Inventory) Take(item string) string {
	...
	if !ok {
		...
		return fmt.Sprintln("взяли предмет", item)
	}
	i.items[item] += 1
	return fmt.Sprintln("увеличили количество")
}
...
func (i *Inventory) Drop(item string) string {
	...
	if !ok {
		...
		return fmt.Sprintln("предмет отсутсвует")
	}
	if i.items[item] == 1 {
		i.items[item] = 0

		return fmt.Sprintln("выбросили")
	}
	i.items[item] -= 1
	return fmt.Sprintln("выбросили 1шт")
}
